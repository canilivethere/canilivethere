...
// The domain address, not the mailbox behind it. hello@ routes through
// Cloudflare Email Routing and forwards on; publishing the forwarding
// DESTINATION instead would have given away the one thing the forwarder
// exists to keep private, and this page already promises "mail lands in
// one plain mailbox" - still true, and now true without naming it.
// Matches .well-known/security.txt, which has always used this address.
const CONTACT_EMAIL = "hello@canilivethere.info";

const MAIL_TEXT =
  "Mail lands in one plain mailbox. There's no list to join, no " +
  "follow-up machinery, and your address goes nowhere else.";

// Gated behind a single flag — this whole page stands complete without
// the claim if it is ever withdrawn; flip this one flag rather than
