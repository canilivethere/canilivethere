...
import { initCostComparison } from "./cost-comparison.js";
...
async function main() {
  // Part 39: FX rates load alongside the store (the location pages'
  // exact pattern) — the cost-comparison panel's bar geometry needs
  ...
  // Part 39: the cost-comparison panel — rendered ONCE, outside
  // render()'s persona/sort/filter re-render cycle on purpose: its
  // content is the general layer, persona-independent and unaffected
  // by the table's own controls (the panel's disclosure line states
  // exactly that), so table re-renders never touch it.
  initCostComparison(store);
...
function renderRow(store, row, persona, tbody, { suppressVerdict = false } = {}) {
  ...
  // whatever hex render time baked in.
  const rampKind = STATE.purposeCriterion ? "score" : "index";
  const rampAttrs = `${RAMP_VALUE_ATTR}="${displayValue == null ? "" : displayValue}" ${RAMP_KIND_ATTR}="${rampKind}"`;
  const fitCellHtml = displayValue != null
    ? `<span class="fit-swatch" style="background:${colorFor(displayValue)}" ${rampAttrs}></span> ${displayValue.toFixed(1)}/5${fallbackTag}${customTag}`
    : `<span class="fit-swatch" style="background:${colorFor(displayValue)}" ${rampAttrs}></span> not scored`;
